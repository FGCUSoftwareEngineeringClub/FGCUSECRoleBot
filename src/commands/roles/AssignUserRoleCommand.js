...
class AssignUserRoleCommand extends Commando.Command { // TODO: Redo all of this.
  ...
  */
  async run(message, args) {
    ...
    const rolesToAssign = this.getRolesToAssign(message, requestedRoles)
        .map((role) => role.toLowerCase());

    const rolesThatWereRequestedAndExistInThisServer =
                                                      this.getRolesOfServer(message, rolesToAssign);

    await this.removeUserConflictingRoles(message, rolesThatWereRequestedAndExistInThisServer);

    /**
     * If the user's current roles don't contain the name of the role requested, place them in this
     * collection.
     */
    const rolesTheUserRequestedAndDoesNotCurrentlyHave =
      rolesThatWereRequestedAndExistInThisServer.filter(function(role) {
        return !message.member.roles.find((findingRole) => findingRole.name === role.name);
      });

    const namesOfServerRolesAssigned = rolesTheUserRequestedAndDoesNotCurrentlyHave
   ...
   * @return {string[]} The roles a user requested.
   */
  getRequestedRoles(message) {
    return message.argString
        ...
        .filter((trimmedString) => trimmedString.length !== 0); // Remove any items empty items.
  }
  ...
  getRolesToAssign(message, requestedRoles) {
    ...
    const nonOverlappingRolesToAssign =
      this.findUserPreexistingNonOverlappingRoles(message, new Map());
    ...
    for (const role of requestedRoles) {
      ...
      } else if (RolesAggregate.doesOverlappingRoleExist(role, true)) {
        // If set doesn't exist, then add it to the list since it should be an overlapping role.
        allRolesToAssign.push(role);
      }
   ...
   *
   * @return {Discord.Collection<string, Discord.Role>}
   */
  getRolesOfServer(message, requestedRoles) {
    ...
    const serverRolesThatAreManagedAndRequested = allServerRoles.filter(function(role) {
      return RolesAggregate.doesRoleExist(role.name, true) &&
            requestedRoles.includes(role.name.toLowerCase());
    });
   ...
   * @return {Map<NonOverlappingRoleSet, string>}
   */
  ...
  /**
   * Removes any roles that a user currently has that would conflict with the roles they're to be
   *  assigned.
   * @param {Commando.CommandMessage} message The message that prompted this command.
   * @param {Discord.Collection<string, Discord.Role>} requestedRoles A collection of requested
   *  roles that exist.
   */
